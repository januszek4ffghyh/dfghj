...
(function () {
    ...
    const DEFAULT_CFG = {
        ...
        // Auto-Relog (Minutnik) & Powrót
        autoRelog: true,
        relogSecondsBefore: 10,  // Ile sekund przed 0s ma klikać relog
        autoReturnToE2: true,    // Automatyczny powrót na E2 (po śmierci / w mieście)
    ...
    // ═══ AUTO-RETURN TO E2 (po śmierci) ═══
    let _needToReturnToE2 = false;
    let _returnPhase = 'idle'; // idle, returning, arrived
    let _returnTargetMapId = null;
    let _returnLastGateAttempt = 0;
    let _deathCount = 0;

    // ═══ ANTI-STASIS ═══
    let _lastStasisCheck = 0;
    let _stasisKickCount = 0;

    // ═══ TRASY POWROTNE DO E2 ═══
    // Każda trasa to tablica map od spawnu (miasto) do celu E2
    const E2_RETURN_ROUTES = {
        // Szczęt alias Gładki (lvl 47) — Stary Kupiecki Trakt
        2308: {
            bossName: 'Szczęt alias Gładki',
            spawnMapId: 33,  // Eder (respawn po śmierci)
            route: [
                { name: 'Eder', id: 33 },
                { name: 'Fort Eder', id: 244 },
                { name: 'Ciemnica Szubrawców p.1 - sala 1', id: 2350 },
                { name: 'Ciemnica Szubrawców p.1 - sala 2', id: 2351 },
                { name: 'Ciemnica Szubrawców p.1 - sala 3', id: 2352 },
                { name: 'Stary Kupiecki Trakt', id: 2308 },
            ]
        },
        // Razuglag Oklash (lvl 51) — Stare Wyrobisko p.3
        125: {
            bossName: 'Razuglag Oklash',
            spawnMapId: 1,  // Ithan (respawn po śmierci)
            route: [
                { name: 'Ithan', id: 1 },
                { name: 'Zniszczone Opactwo', id: 8 },
                { name: 'Zburzona Twierdza', id: 116 },
                { name: 'Nawiedzony Jar', id: 122 },
                { name: 'Stare Wyrobisko p.5', id: 127 },
                { name: 'Stare Wyrobisko p.4', id: 126 },
                { name: 'Stare Wyrobisko p.3', id: 125 }
            ]
        }
    };
...
function waitForEngine(cb) {
    const check = () => {
        if (window.Engine && Engine.hero && Engine.hero.d && Engine.npcs) {
            ...
            console.log('[E2H] Engine ready → włączam Auto Return to E2 (dashboard)');

            // Główny monitoring flagi z dashboardu
            setInterval(() => {
                if (window.AUTO_RETURN_TO_E2 === true) {
                    console.log('[E2H] Dashboard włączył Auto Return — uruchamiam tryReturnToE2()');
                    tryReturnToE2();
                }
            }, 4000);
    ...
    function scanGateways() {
        ...
        // 1. Nowy silnik: g.gwIds & g.townname
        if (window.g && window.g.gwIds) {
            try {
                const gwIds = window.g.gwIds;
                const townname = window.g.townname || {};
                const gw2 = window.g.gw2 || {}; // opcjonalnie, czasem zawiera ID map docelowych
                ...
                for (const id in gwIds) {
                    ...
                    if (Number.isFinite(tx) && Number.isFinite(ty)) {
                        // Pobierz nazwę mapy docelowej
                        const name = (townname[id] || `Brama #${id}`).trim();
                        // Pobierz ID mapy docelowej (z g.gw2 jeśli istnieje)
                        let targetMapId = 0;
                        if (gw2[id]) {
                            targetMapId = typeof gw2[id] === 'object' ? (gw2[id].id || gw2[id].map || 0) : gw2[id];
                        }
                        
                        gateways.push({
                            targetMapId: Number(targetMapId),
                            tx,
                            ty,
                            name
                        });
                ...
                console.error('[E2H:scanGateways] Błąd nowego silnika:', e);
            }
        }

        // 2. Fallback / Uzupełnienie: Engine.map i Engine.map.d
        if (window.Engine && Engine.map) {
            try {
                ...
                sources.forEach(src => {
                    if (!src) return;
                    const gates = src.gateways || src.gates || src.gw || src.gw2;
                    if (!gates) return;
                    
                    // Bezpieczna konwersja na tablicę, obsługa getList() dla Engine.map.gateways
                    let gateArr = [];
                    if (typeof gates.getList === 'function') {
                        gateArr = gates.getList();
                    } else if (Array.isArray(gates)) {
                        gateArr = gates;
                    } else {
                        // Sprawdzamy czy to nie jest słownik "townnames" w Luvia (który ignorujemy, bo nie ma pozycji)
                        if (gates.townnames && Object.keys(gates).length === 1) {
                            return; 
                        }
                        gateArr = Object.values(gates);
                    }

                    gateArr.forEach(gate => {
                        if (!gate) return;
                        const targetMapId = gate.targetMapId || gate.toMap || gate.map || gate.id2 || gate.id || (gate.d && gate.d.id) || 0;
                        
                        // Zabezpieczenie przed błędnym typem mapId (np. nazwa zamiast liczby)
                        const parsedId = Number(targetMapId);
                        
                        const tx = gate.x || gate.tx || gate.rx || (gate.d && gate.d.x) || 0;
                        const ty = gate.y || gate.ty || gate.ry || (gate.d && gate.d.y) || 0;
                        
                        let name = gate.name || '';
                        if (!name && gate.tip) {
                            name = Array.isArray(gate.tip) ? gate.tip[0] : String(gate.tip);
                        }
                        if (!name) {
                            name = `Mapa #${parsedId}`;
                        }
                        name = name.replace(/<[^>]*>/g, '').trim(); // czyszczenie HTML
                        
                        // Zapobiegaj dodawaniu pustych/uszkodzonych bram (0,0)
                        if (tx === 0 && ty === 0) return;

                        // Zapobiegaj duplikatom
                        const existing = gateways.find(g => Math.abs(g.tx - tx) <= 1 && Math.abs(g.ty - ty) <= 1);
                        if (!existing) {
                            gateways.push({
                                targetMapId: Number.isFinite(parsedId) ? parsedId : 0,
                                tx, ty, name
                            });
                        } else {
                            if (!existing.targetMapId && parsedId) {
                                existing.targetMapId = Number.isFinite(parsedId) ? parsedId : 0;
                            }
                        }
                    });
                });
            } catch(e) {
                console.error('[E2H:scanGateways] Błąd skanowania Engine:', e);
            }
        }

        // 3. Fallback: DOM
        document.querySelectorAll('.gw').forEach(el => {
            const idMatch = el.className.match(/gwmap(\d+)/) || el.id?.match(/gw(\d+)/);
            if (!idMatch) return;
            ...
            const tip = el.getAttribute('tip') || el.getAttribute('data-tip') || '';
            ...
            if (!existing) {
                ...
                if (!existing.targetMapId) {
                    existing.targetMapId = targetMapId;
                }
            }
        });
    ...
    function isValidTile(x, y) {
        ...
        const w = window.Engine.map.d.w || window.Engine.map.d.x || 100;
        const h = window.Engine.map.d.h || window.Engine.map.d.y || 100;
        if (x < 0 || x >= w || y < 0 || y >= h) return false;
        if (window.Engine.map.isCol && window.Engine.map.isCol(x, y)) return false;
        return true;
    }

    function isSzczetZone(x, y) {
        if (y < 74 || y > 79) return false;
        if (y === 74 || y === 75) return (x >= 7 && x <= 16);
        if (y === 76 || y === 77) return (x >= 7 && x <= 15);
        if (y === 78 || y === 79) return (x >= 8 && x <= 14);
        return false;
    }

    function getRandomSzczetZoneTile() {
        const candidateTiles = [];
        for (let y = 74; y <= 79; y++) {
            let minX = 7, maxX = 15;
            if (y === 74 || y === 75) { minX = 7; maxX = 16; }
            else if (y === 76 || y === 77) { minX = 7; maxX = 15; }
            else if (y === 78 || y === 79) { minX = 8; maxX = 14; }
    ...
    function isRazuglagZone(x, y) {
        // Box 1: x in [5, 8], y in [4, 11]
        const inBox1 = (x >= 5 && x <= 8 && y >= 4 && y <= 11);
        // Box 2: x in [3, 10], y === 7
        const inBox2 = (x >= 3 && x <= 10 && y === 7);
        return inBox1 || inBox2;
    }

    function getRandomRazuglagZoneTile() {
        const candidateTiles = [];
        // Box 1
        for (let x = 5; x <= 8; x++) {
            for (let y = 4; y <= 11; y++) {
                if (isValidTile(x, y)) candidateTiles.push({ x, y });
            }
        }
        // Box 2
        for (let x = 3; x <= 10; x++) {
            const y = 7;
            if (isValidTile(x, y)) {
                // Unikaj duplikatów z box1 (gdzie x jest 5..8 i y jest 7)
                if (!(x >= 5 && x <= 8)) {
                    candidateTiles.push({ x, y });
                }
            }
        ...
        return { x: 7, y: 7 }; // Safe fallback
    }

    // ═══════════════════════════════════════════════════════════════
    //  A* PATHFINDING + KOLIZJE (z bota expowiskowego)
    // ═══════════════════════════════════════════════════════════════
    let _cachedColMethod = null;
    let _colCacheMapId = null;

    /**
     * Wykrywa metodę sprawdzania kolizji w Engine.map.col
     * Obsługuje wiele formatów: check(), get(), string col, 2D array
     */
    function _detectCollisionMethod(w, h) {
        let col = Engine.map.col;
        if (!col) {
            // Fallback: Engine.map.d.col (string)
            if (Engine.map.d && Engine.map.d.col && typeof Engine.map.d.col === 'string') {
                const colStr = Engine.map.d.col;
                return (x, y) => colStr.charAt(x + y * w) !== '0';
            }
            return null;
        }
        ...
        // Szukaj ukrytych danych kolizji w obiekcie col
        for (let key in col) {
            let data = col[key];
    ...
    /**
     * Sprawdza czy pole (x,y) jest zablokowane (ściana/kolizja)
     */
    function isTileBlocked(x, y) {
        if (typeof Engine === 'undefined' || !Engine.map || !Engine.map.d) return true;
        const w = Engine.map.d.w || Engine.map.d.x || 100;
        const h = Engine.map.d.h || Engine.map.d.y || 100;

        if (x < 0 || y < 0 || x >= w || y >= h) return true;

        // Cache kolizji per mapa
        const currentId = Engine.map.d.id;
        if (_colCacheMapId !== currentId) {
            _cachedColMethod = _detectCollisionMethod(w, h);
            _colCacheMapId = currentId;
        }

        if (_cachedColMethod) return _cachedColMethod(x, y);

        // Ostateczny fallback: Engine.map.isCol
        if (Engine.map.isCol) return Engine.map.isCol(x, y);

        return false;
    }

    /**
     * A* pathfinding — znajduje ścieżkę od (startX,startY) do (celX,celY)
     * omijając ściany. Zwraca tablicę [{x,y},...] lub null jeśli brak ścieżki.
     */
    function findPath(startX, startY, celX, celY) {
        ...
        while (openSet.length > 0) {
            iterations++;
            if (iterations > 2000) return null; // za daleko

            // Znajdź node z najniższym f
            let minIndex = 0;
            ...
            for (let s of neighbors) {
                // Cel jest zawsze dozwolony (brama może być na "zablokowanym" polu)
                if (isTileBlocked(s.x, s.y) && !(s.x === celX && s.y === celY)) continue;
    ...
    // ═══════════════════════════════════════════════════════════════
    //  STEP-BY-STEP GATEWAY WALKING (krok po kroku do bramy)
    // ═══════════════════════════════════════════════════════════════
    let _gatewayPath = [];
    let _gatewayPathIndex = 0;
    let _gatewayWalking = false;
    let _lastGatewayStep = 0;

    /**
     * Rozpoczyna chodzenie do bramy krokami (z A* pathfinding)
     * @returns true jeśli rozpoczęto, false jeśli nie znaleziono ścieżki
     */
    function startWalkToGateway(gateX, gateY) {
        const hero = getHeroInfo();
        if (!hero) return false;

        const path = findPath(hero.x, hero.y, gateX, gateY);
        if (!path || path.length === 0) {
            // Fallback: spróbuj autoGoTo
            log(`⚠️ A* nie znalazł ścieżki do [${gateX},${gateY}], próbuję autoGoTo...`, 'warn');
            try {
                Engine.hero.autoGoTo({ x: gateX, y: gateY }, false);
            } catch {}
            return true;
        }
        ...
        _gatewayPath = path;
        _gatewayPathIndex = 0;
        _gatewayWalking = true;
        _lastGatewayStep = 0;
        log(`🛤️ A* znalazł ścieżkę: ${path.length} kroków do bramy [${gateX},${gateY}]`, 'info');
        return true;
    }

    /**
     * Wykonuje kolejny krok na ścieżce do bramy.
     * Wywoływany co tick. Robi ~4 pola na raz (jak bot expowiskowy).
     * @returns 'walking' jeśli dalej idzie, 'done' jeśli dotarł/anulowano
     */
    function doGatewayWalkStep() {
        if (!_gatewayWalking || _gatewayPath.length === 0) return 'done';

        const now = Date.now();
        if (now - _lastGatewayStep < 600) return 'walking'; // min 600ms między krokami
        _lastGatewayStep = now;

        // Ile kroków na raz (jak KROKOW_SKOK w bocie exp)
        const stepsPerTick = 4;
        const targetIdx = Math.min(_gatewayPathIndex + stepsPerTick, _gatewayPath.length - 1);
        const target = _gatewayPath[targetIdx];

        try {
            Engine.hero.autoGoTo({ x: target.x, y: target.y });
        } catch (e) {
            log(`❌ Błąd kroku A*: ${e.message}`, 'err');
            _gatewayWalking = false;
            return 'done';
        }

        _gatewayPathIndex = targetIdx + 1;

        if (_gatewayPathIndex >= _gatewayPath.length) {
            _gatewayWalking = false;
            log(`✅ Dotarłem do bramy! Czekam na zmianę mapy...`, 'ok');
            return 'done';
        }
    ...
    /**
     * Anuluje chodzenie do bramy
     */
    function cancelGatewayWalk() {
        _gatewayWalking = false;
        _gatewayPath = [];
        _gatewayPathIndex = 0;
    }
    ...
    function triggerAntiBanMovement() {
        try {
            ...
            const isSzczetMap = (mapInfo.id === 2308);
            const isRazuglagMap = (mapInfo.id === 125);

            // Ustaw pozycję startową (domową) jako bazę powrotu, jeśli jeszcze jej nie ma
            if (!_movementHome) {
                if (isSzczetMap) {
                    _movementHome = getRandomSzczetZoneTile();
                } else if (isRazuglagMap) {
                    _movementHome = getRandomRazuglagZoneTile();
                } else {
                    _movementHome = { x: hero.x, y: hero.y };
                }
            } else if (isSzczetMap && !isSzczetZone(_movementHome.x, _movementHome.y)) {
                _movementHome = getRandomSzczetZoneTile();
            } else if (isRazuglagMap && !isRazuglagZone(_movementHome.x, _movementHome.y)) {
                _movementHome = getRandomRazuglagZoneTile();
            }
            ...
            if (rand < 0.35) {
                ...
                if (isValidTile(stepX, stepY) && 
                    (!isSzczetMap || isSzczetZone(stepX, stepY)) && 
                    (!isRazuglagMap || isRazuglagZone(stepX, stepY)) && 
                    isPathClear(_movementHome.x, _movementHome.y, stepX, stepY)) {
                ...
                for (const pos of path) {
                    if (!isValidTile(pos.x, pos.y) || 
                        (isSzczetMap && !isSzczetZone(pos.x, pos.y)) || 
                        (isRazuglagMap && !isRazuglagZone(pos.x, pos.y)) || 
                        !isPathClear(lastPos.x, lastPos.y, pos.x, pos.y)) {
                        pathOk = false;
                        break;
                    }
                    lastPos = pos;
            ...
            if (queue.length === 0) {
                ...
                for (let i = 0; i < 20; i++) {
                    ...
                    if (isValidTile(tx, ty) && 
                        (!isSzczetMap || isSzczetZone(tx, ty)) && 
                        (!isRazuglagMap || isRazuglagZone(tx, ty)) && 
                        isPathClear(_movementHome.x, _movementHome.y, tx, ty)) {
    ...
    // ═══════════════════════════════════════════════════════════════
    //  AUTO-RETURN TO E2 (po śmierci — nawigacja powrotna)
    // ═══════════════════════════════════════════════════════════════

    /**
     * Aktywuje powrót do E2 po śmierci.
     * Szuka trasy na podstawie filtru nazw E2 lub bieżącej lokalizacji.
     */
    async function activateReturnToE2() {
        await fetchConfig();
        // Znajdź docelową mapę E2 z filtru nazw
        let targetRoute = null;
        let targetMapId = null;

        // 1. Sprawdź najpierw przypisanego bossa dla aktualnej postaci w char_boss_map z serwera
        const hero = getHeroInfo();
        log(`🔍 activateReturnToE2: hero=${hero?.nick}, serverConfig keys=${Object.keys(_serverConfig || {}).join(',')}, char_boss_map=${JSON.stringify(_serverConfig?.char_boss_map)}`, 'info');
        if (hero && hero.nick && hero.nick !== '?' && _serverConfig && _serverConfig.char_boss_map) {
            try {
                let mapping = _serverConfig.char_boss_map;
                if (typeof mapping === 'string') {
                    mapping = JSON.parse(mapping);
                }
                if (mapping && typeof mapping === 'object') {
                    const assignedBoss = mapping[hero.nick.toLowerCase()];
                    log(`🔍 char_boss_map lookup: nick="${hero.nick.toLowerCase()}" → boss="${assignedBoss || 'nie znaleziono'}"`, 'info');
                    if (assignedBoss) {
                        for (const [mapId, routeData] of Object.entries(E2_RETURN_ROUTES)) {
                            if (routeData.bossName.toLowerCase() === assignedBoss.toLowerCase()) {
                                targetRoute = routeData;
                                targetMapId = parseInt(mapId);
                                log(`✅ Dopasowano trasę z char_boss_map: ${routeData.bossName} (mapId=${mapId})`, 'ok');
                                break;
                            }
                        }
                        if (!targetRoute) {
                            log(`⚠️ Boss "${assignedBoss}" z char_boss_map nie ma trasy w E2_RETURN_ROUTES!`, 'warn');
                        }
                    }
                }
            } catch (e) {
                console.error('[E2H:activateReturnToE2] Error parsing char_boss_map:', e);
            }
        }

        // 2. Sprawdź filtr E2 w konfiguracji
        if (!targetRoute && cfg.e2NameFilter) {
            const filters = cfg.e2NameFilter.split(',').map(f => f.trim().toLowerCase()).filter(f => f.length > 0);
            for (const e2 of E2_DATABASE) {
                if (filters.some(f => e2.name.toLowerCase().includes(f))) {
                    // Znajdź trasę dla tej mapy
                    for (const [mapId, routeData] of Object.entries(E2_RETURN_ROUTES)) {
                        if (routeData.bossName.toLowerCase() === e2.name.toLowerCase() || parseInt(mapId) === e2.mapId) {
                            targetRoute = routeData;
                            targetMapId = parseInt(mapId);
                            break;
                        }
        ...
        // 3. Fallback: szukaj po mapie E2 z bazy
        if (!targetRoute) {
            // Próbuj znaleźć wg ostatniej mapy
            const lastMap = localStorage.getItem('e2h_last_e2_map');
            if (lastMap) {
                const mapId = parseInt(lastMap);
                if (E2_RETURN_ROUTES[mapId]) {
                    targetRoute = E2_RETURN_ROUTES[mapId];
                    targetMapId = mapId;
                }
            }
        }

        // 4. Domyślnie: Szczęt alias Gładki (2308)
        if (!targetRoute) {
            targetMapId = 2308;
            targetRoute = E2_RETURN_ROUTES[2308];
        }

        if (!targetRoute) {
            log('❌ Brak trasy powrotnej do E2! Nie mogę wrócić.', 'err');
            return;
        }

        _needToReturnToE2 = true;
        _returnPhase = 'returning';
        _returnTargetMapId = targetMapId;
        _returnLastGateAttempt = 0;

        log(`🔄 POWRÓT DO E2: ${targetRoute.bossName} (mapa: ${targetRoute.route[targetRoute.route.length - 1].name})`, 'warn');
        log(`📍 Trasa: ${targetRoute.route.map(r => r.name).join(' → ')}`, 'info');
    }

    /**
     * Jeden krok nawigacji powrotnej do E2.
     * Wywoływany co tick w botTick gdy _needToReturnToE2 === true.
     * @returns 'walking' jeśli idziemy do bramy, 'waiting' jeśli czekamy, 'done' jeśli dotarliśmy
     */
    function doReturnToE2Step() {
        // Jeśli aktualnie idziemy A* do bramy, kontynuuj
        if (_gatewayWalking) {
            const stepResult = doGatewayWalkStep();
            if (stepResult === 'walking') return 'walking';
            // 'done' = dotarliśmy do bramy, czekamy na zmianę mapy
            return 'waiting';
        }
        const mapInfo = getMapInfo();
        const currentMapId = mapInfo.id;

        // Znajdź trasę
        const routeData = E2_RETURN_ROUTES[_returnTargetMapId];
        if (!routeData) {
            log('❌ Trasa powrotna nie znaleziona!', 'err');
            _needToReturnToE2 = false;
            _returnPhase = 'idle';
            return 'done';
        }

        // Sprawdź czy już jesteśmy na celu
        if (currentMapId === _returnTargetMapId) {
            log(`✅🎯 DOTARLIŚMY na mapę E2: ${mapInfo.name}!`, 'ok');
            _needToReturnToE2 = false;
            _returnPhase = 'arrived';
            return 'done';
        }

        // Znajdź obecną pozycję na trasie
        const route = routeData.route;
        const currentIdx = route.findIndex(r => r.id === currentMapId);

        if (currentIdx === -1) {
            // Nie jesteśmy na żadnej mapie z trasy — może inne miasto/respawn
            log(`⚠️ Powrót: obecna mapa (${mapInfo.name}, ID:${currentMapId}) nie jest na trasie. Szukam bramy...`, 'warn');
            
            // Spróbuj znaleźć bramę do PIERWSZEJ mapy na trasie (miasto startowe)
            const gateways = scanGateways();
            const firstMapOnRoute = route[0];
            const gate = gateways.find(g => g.targetMapId === firstMapOnRoute.id || g.name.toLowerCase().includes(firstMapOnRoute.name.toLowerCase()));
            
            if (gate) {
                phase = 'walking';
                log(`🚶 Powrót: idę do bramy → ${gate.name} [${gate.tx}, ${gate.ty}] (A*)`, 'info');
                startWalkToGateway(gate.tx, gate.ty);
                return 'walking';
            } else {
                log(`❌ Nie znaleziono bramy do mapy startowej (${firstMapOnRoute.name})`, 'err');
                return 'waiting';
            }
        }

        // Kolejna mapa na trasie
        const nextIdx = currentIdx + 1;
        if (nextIdx >= route.length) {
            // Nie powinno się zdarzyć (sprawdzamy na początku), ale na wszelki wypadek
            _needToReturnToE2 = false;
            _returnPhase = 'arrived';
            return 'done';
        }

        const nextMapOnRoute = route[nextIdx];

        // Nie szukaj bramy zbyt często (co min 3s)
        const now = Date.now();
        if (now - _returnLastGateAttempt < 3000) {
            return 'waiting';
        }
        _returnLastGateAttempt = now;

        // Szukaj bramy do następnej mapy
        const gateways = scanGateways();
        log(`🔍 Powrót: szukam bramy do [${nextMapOnRoute.name}] (ID: ${nextMapOnRoute.id}). Znalezione bramy: ${gateways.length}`, 'info');

        // Szukaj po ID mapy docelowej
        let gate = gateways.find(g => g.targetMapId === nextMapOnRoute.id);

        // Fallback: szukaj po nazwie
        if (!gate) {
            gate = gateways.find(g => g.name.toLowerCase().includes(nextMapOnRoute.name.toLowerCase()));
        }

        // Fallback 2: szukaj po częściowej nazwie (bez "p.1 - sala X")
        if (!gate) {
            const shortName = nextMapOnRoute.name.split(' - ')[0].split(' p.')[0].toLowerCase();
            gate = gateways.find(g => g.name.toLowerCase().includes(shortName));
        }

        if (gate) {
            phase = 'walking';
            log(`🚶 Powrót [${currentIdx + 1}/${route.length}]: A* do bramy → ${gate.name} [${gate.tx}, ${gate.ty}]`, 'warn');
            startWalkToGateway(gate.tx, gate.ty);
            return 'walking';
        } else {
            log(`⚠️ Brak bramy do ${nextMapOnRoute.name} na tej mapie! Bramy znalezione: ${gateways.map(g => `${g.name}(→${g.targetMapId})`).join(', ') || 'brak'}`, 'err');
            return 'waiting';
        }
    }

    // ═══════════════════════════════════════════════════════════════
    //  ANTI-STASIS (zapobieganie stanowi nieaktywności)
    // ═══════════════════════════════════════════════════════════════

    /**
     * Sprawdza czy wyświetla się overlay "stan nieaktywności"
     * i jeśli tak — robi krok do przodu, żeby postać pozostała aktywna.
     */
    function checkAndPreventStasis() {
        const now = Date.now();
        if (now - _lastStasisCheck < 3000) return; // nie sprawdzaj częściej niż co 3s
        _lastStasisCheck = now;

        try {
            // Detekcja: .stasis-incoming-overlay (ostrzeżenie) ORAZ .stasis-overlay (sam stan nieaktywności)
            const stasisOverlay = document.querySelector('.stasis-overlay, .stasis-incoming-overlay');
            if (!stasisOverlay) return;

            const style = window.getComputedStyle(stasisOverlay);
            if (style.display === 'none' || style.visibility === 'hidden') return;

            // STASIS WYKRYTY! Robimy krok
            _stasisKickCount++;
            log(`⚡ ANTI-STASIS #${_stasisKickCount}: Wykryto ostrzeżenie o nieaktywności! Robię ruch...`, 'warn');

            // Pobierz pozycję bohatera i idź o 1-2 pola
            const hero = getHeroInfo();
            if (hero) {
                const dx = Math.floor(Math.random() * 3) - 1; // -1, 0, 1
                const dy = Math.floor(Math.random() * 3) - 1;
                const targetX = hero.x + (dx || 1); // zawsze przynajmniej 1 pole
                const targetY = hero.y + (dy || 1);

                try {
                    Engine.hero.autoGoTo({ x: targetX, y: targetY });
                    log(`⚡ ANTI-STASIS: Ruch do [${targetX}, ${targetY}]`, 'info');
                } catch (e) {
                    // Fallback: spróbuj po prostu ruszyć się w losowym kierunku
                    try {
                        walkToRandomNearPosition(2);
                    } catch {}
                }
            }
        } catch (e) {
            console.error('[E2H] checkAndPreventStasis error:', e);
        }
    }
    ...
    async function fetchConfig() {
        ...
        try {
            const res = await fetch(`${window.MAW_NODE_API}/api/config`);
            if (res.ok) {
                const data = await res.json();
                if (data && data.settings) {
                    _serverConfig = data.settings;
                }
            }
        } catch (e) {
            console.error('[E2H] Error fetching config:', e);
        }
    }
    ...
    function findCharacterForE2(e2Name) {
        ...
        // 1. Sprawdź najpierw w mapowaniu char_boss_map z API serwera
        if (_serverConfig && _serverConfig.char_boss_map) {
            try {
                let mapping = _serverConfig.char_boss_map;
                if (typeof mapping === 'string') {
                    mapping = JSON.parse(mapping);
                ...
                if (mapping && typeof mapping === 'object') {
                    for (const [charNick, boss] of Object.entries(mapping)) {
                        if (boss && boss.toLowerCase() === cleanE2Name.toLowerCase()) {
                            // Znaleźliśmy postać przypisaną do tego bossa!
                            const matchedChar = lastAccountCharacters.find(c => c.nick.toLowerCase() === charNick.toLowerCase());
                            if (matchedChar) {
                                return matchedChar;
                            }
                            // Jeśli postać nie jest w lastAccountCharacters, i tak zwróć jej nick z przypisania!
                            return { nick: charNick };
                        }
                    }
                }
            } catch (e) {
                console.error('[E2H:findCharacterForE2] Error parsing char_boss_map:', e);
            }
        }
        ...
        // 2. Sprawdź w lokalnym rejestrze map postaci (zapisywanym na bieżąco w grze)
    ...
    async function scrapeCharactersFromReloggerDOM() {
        // Zobacz czy okno reloggera jest otwarte
        let relogger = document.querySelector('.relogger-window');
        let weOpenedIt = false;
        if (!relogger) {
            // Spróbuj otworzyć klikając hamburgera
            const burger = document.querySelector('.manage-hamburger-button');
            if (burger) {
                burger.click();
                await new Promise(r => setTimeout(r, 250));
                relogger = document.querySelector('.relogger-window');
                weOpenedIt = true;
            }
        }
        if (!relogger) return null;

        // Tymczasowo upewnij się że WSZYSTKIE grupy światów są widoczne w DOM (by nie pominąć nieaktywnych zakładek)
        const charGroups = relogger.querySelectorAll('.relogger__char-group');
        const prevGroupStates = [];
        charGroups.forEach(g => {
            prevGroupStates.push({ el: g, display: g.style.display, active: g.classList.contains('active') });
        ...
        const scraped = [];
        const charElements = relogger.querySelectorAll('.relogger__one-character');
        for (const char of charElements) {
            const groupEl = char.closest('.relogger__char-group');
            const groupWorld = groupEl ? (groupEl.getAttribute('data-world') || '') : '';
            const tipId = char.getAttribute('tip-id');

            let tipText = '';
            
            // 1. Sprawdź w pamięci tips gry
            if (tipId && window.Engine?.tips) {
                try {
                    const tData = window.Engine.tips.get ? window.Engine.tips.get(tipId) : window.Engine.tips.data?.[tipId];
                    if (tData) tipText = typeof tData === 'string' ? tData : (tData.html || tData.text || tData.content || '');
                } catch {}
            }
            if (!tipText && tipId && window.g?.tips?.[tipId]) {
                tipText = String(window.g.tips[tipId]);
            }

            // 2. Fallback: mouseenter na widocznym elemencie
            if (!tipText) {
                char.dispatchEvent(new MouseEvent('mouseenter', { bubbles: true }));
                await new Promise(r => setTimeout(r, 60));
                const tooltip = document.querySelector('.tip-wrapper, .normal-tip, #tooltip, .tooltip, .tip-content');
                if (tooltip) {
                    tipText = tooltip.textContent.trim();
                }
                char.dispatchEvent(new MouseEvent('mouseleave', { bubbles: true }));
            }

            // Oczyszczenie HTML jeśli tipText zawiera tagi
            if (tipText.includes('<')) {
                const tmp = document.createElement('div');
                tmp.innerHTML = tipText;
                tipText = tmp.textContent.trim();
            }
            ...
            let prof = '';
            let lvl = 1;
            const lvlMatch = secondLine.match(/lvl[:\s]+(\d+)/i) || secondLine.match(/(\d+)\s*poziom/i);
            if (lvlMatch) {
                lvl = parseInt(lvlMatch[1], 10);
            }
            const profMatch = secondLine.split(',')[0].trim();
            if (profMatch) {
                prof = profMatch;
            }

            let world = groupWorld;
            const worldLine = lines.find(l => l.toLowerCase().includes('świat:'));
            if (worldLine) {
                world = worldLine.replace(/świat:\s*/i, '').trim();
            }

            let id = tipId || '';
            const idLine = lines.find(l => l.toLowerCase().includes('id:'));
            if (idLine) {
                id = idLine.replace(/id:\s*/i, '').trim();
            }

            const imgStyle = char.querySelector('.img-avatar-correct')?.style.backgroundImage || '';
            let avatarUrl = '';
            if (imgStyle) {
        ...
        // Przywróć stany grup
        prevGroupStates.forEach(s => {
            s.el.style.display = s.display;
            if (!s.active) s.el.classList.remove('active');
        });

        if (weOpenedIt) {
            const closeBtn = relogger.querySelector('.close-button');
    ...
    async function fetchAccountCharacters() {
        ...
        // 1. Spróbuj najpierw pobrać postacie z reloggera w grze
        try {
            const localChars = await scrapeCharactersFromReloggerDOM();
            if (localChars && localChars.length > 0) {
                lastAccountCharacters = localChars;
                log(`👥 Wczytano listę postaci z reloggera w grze (${lastAccountCharacters.length} postaci)`, 'info');
                if (window.MAW_NODE_API) {
                    fetch(`${window.MAW_NODE_API}/api/chars`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(localChars)
                    }).catch(() => {});
                }
                
                const container = document.getElementById('e2h-char-boss-container');
                if (container && container.style.display === 'block' && typeof window.renderCharBossTable === 'function') {
                    window.renderCharBossTable();
                }
                return;
            }
        } catch (e) {
            console.error('[E2H] Błąd skrapowania z reloggera w grze:', e);
        }

        // 2. Fallback na publiczne API Margonem
        try {
            ...
            if (Array.isArray(data)) {
                ...
                log(`👥 Wczytano listę postaci z API konta (${lastAccountCharacters.length} postaci)`, 'info');
                if (window.MAW_NODE_API) {
                    fetch(`${window.MAW_NODE_API}/api/chars`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
            ...
            console.error('[E2H] Błąd wczytywania postaci z API konta:', e);
        }
    }
    ...
    function botTick() {
        ...
        try {
                ...
                if (isBattleOver()) {
                    if (!_battleProcessed) {
                        ...
                        if (won) {
                            ...
                            _deathCount++;
                            log(`💀 Walka PRZEGRANA! (zgony: ${_deathCount})`, 'err');
                            if (cfg.autoReturnToE2 !== false) {
                                log(`🔄 Auto-powrót na E2 aktywowany!`, 'warn');
                                activateReturnToE2();
                            }
                        }
            ...
            // ═══ ANTI-STASIS: wykryj overlay nieaktywności i zrób krok ═══
            checkAndPreventStasis();
            ...
            if (mapInfo.id !== lastMapId) {
                ...
                // Zapisz mapę E2 jeśli mamy dla niej trasę powrotną
                if (E2_RETURN_ROUTES[mapInfo.id]) {
                    localStorage.setItem('e2h_last_e2_map', String(mapInfo.id));
                }

                // ═══ AUTO-RETURN: sprawdź czy dotarliśmy na docelową mapę ═══
                if (_needToReturnToE2 && _returnTargetMapId) {
                    if (mapInfo.id === _returnTargetMapId) {
                        log(`✅🎯 POWRÓT ZAKOŃCZONY! Dotarliśmy na mapę E2: ${mapInfo.name}`, 'ok');
                        _needToReturnToE2 = false;
                        _returnPhase = 'arrived';
                        _returnTargetMapId = null;
                    } else {
                        log(`🔄 Powrót: zmiana mapy → ${mapInfo.name} (cel: ID ${_returnTargetMapId})`, 'info');
                    }
                } else if (cfg.autoReturnToE2 !== false && !_needToReturnToE2 && botRunning) {
                    // Sprawdź czy jesteśmy w mieście startowym (np. Ithan, Eder, Torneg, Karka-han) po zgonie/relogu
                    const CITY_MAP_IDS = [1, 33, 10, 20, 244]; // Ithan, Eder, Torneg, Karka-han, Fort Eder
                    if (CITY_MAP_IDS.includes(mapInfo.id)) {
                        log(`📍 Wykryto miasto (${mapInfo.name}) — auto-powrót na E2 uruchomiony!`, 'warn');
                        activateReturnToE2();
                    }
                }
            }

            // ═══ RĘCZNA NAWIGACJA (A*) DO BRAMY ═══
            if (_gatewayWalking) {
                const stepResult = doGatewayWalkStep();
                if (stepResult === 'walking') {
                    renderAll();
                    return; // Nie skanuj mobów, idź do bramy
                }
            }

            // ═══ AUTO-RETURN TO E2: nawigacja po trasie ═══
            if (_needToReturnToE2 && _returnPhase === 'returning') {
                const returnResult = doReturnToE2Step();
                if (returnResult === 'walking') {
                    renderAll();
                    return; // Nie skanuj mobów, idź do bramy
                }
            }
            ...
            if (targets.length === 0) {
                ...
                if (mapInfo.id === 2308) {
                    const hero = getHeroInfo();
                    if (hero && hero.nick !== '?') {
                        if (!isSzczetZone(hero.x, hero.y)) {
                            if (_movementQueue.length === 0 && !_isExecutingMovement) {
                                log(`🚶 Poza strefą Szczęt alias Gładki [${hero.x},${hero.y}]. Idę do strefy...`, 'info');
                                const targetZoneTile = getRandomSzczetZoneTile();
                                _movementHome = targetZoneTile;
                                const path = findPath(hero.x, hero.y, targetZoneTile.x, targetZoneTile.y);
                                if (path && path.length > 0) {
                                    _movementQueue = path;
                                    executeMovementQueue();
                                } else {
                                    log(`⚠️ A* nie znalazł ścieżki do strefy Szczęt [${targetZoneTile.x},${targetZoneTile.y}], próbuję autoGoTo...`, 'warn');
                                    if (window.Engine && window.Engine.hero && window.Engine.hero.autoGoTo) {
                                        window.Engine.hero.autoGoTo({ x: targetZoneTile.x, y: targetZoneTile.y });
                                    }
                                }
                            }
                            renderAll();
                ...
                if (mapInfo.id === 125) {
                    const hero = getHeroInfo();
                    if (hero && hero.nick !== '?') {
                        if (!isRazuglagZone(hero.x, hero.y)) {
                            if (_movementQueue.length === 0 && !_isExecutingMovement) {
                                log(`🚶 Poza strefą Razuglag Oklash [${hero.x},${hero.y}]. Idę do strefy...`, 'info');
                                const targetZoneTile = getRandomRazuglagZoneTile();
                                _movementHome = targetZoneTile;
                                const path = findPath(hero.x, hero.y, targetZoneTile.x, targetZoneTile.y);
                                if (path && path.length > 0) {
                                    _movementQueue = path;
                                    ...
                                    log(`⚠️ A* nie znalazł ścieżki do strefy Razuglag [${targetZoneTile.x},${targetZoneTile.y}], próbuję autoGoTo...`, 'warn');
                                    if (window.Engine && window.Engine.hero && window.Engine.hero.autoGoTo) {
                                        window.Engine.hero.autoGoTo({ x: targetZoneTile.x, y: targetZoneTile.y });
                                    }
                                }
...
.e2h-ai-led.busy { background: #fbbf24; color: #fbbf24; animation: e2h-pulse 0.4s infinite; }
    ...
    function buildPanel() {
        ...
        <div class="e2h-tab-page active" data-page="hunt">
            ...
            <!-- Auto-Relog & Powrót -->
            <div class="e2h-card">
                <div class="e2h-card-title">⏱️ Auto-Relog & Powrót na E2</div>
                ...
                <div class="e2h-sw"><span class="e2h-sw-label">🔄 Auto-powrót (miasto / śmierć)</span><label class="e2h-toggle"><input type="checkbox" id="e2h-autoReturnToE2" ${cfg.autoReturnToE2 !== false ?'checked':''}><span class="sl"></span></label></div>
            ...
            <!-- E2 Mapping per Character -->
            <div class="e2h-card">
                <div class="e2h-card-title" id="e2h-toggle-char-boss-table" style="cursor:pointer; display:flex; justify-content:space-between; align-items:center;">
                    <span>👥 Przypisz E2 do postaci</span>
                    <span id="e2h-char-boss-arrow" style="font-size:10px">▶</span>
                </div>
                <div id="e2h-char-boss-container" style="display:none; margin-top:8px;">
                    <div style="font-size:10px; color:#94a3b8; margin-bottom:6px;">Wybierz E2 dla każdej postaci:</div>
                    <div id="e2h-char-boss-rows" style="display:flex; flex-direction:column; gap:4px;">
                        <div style="font-size:10px;color:#64748b">Wczytywanie postaci...</div>
                    </div>
                </div>
            </div>
            ...
            <!-- Sąsiednie mapy -->
            <div class="e2h-card">
                <div class="e2h-card-title">🗺️ Obecna mapa: <span id="e2h-current-map" style="color:#60a5fa">?</span> <span id="e2h-current-map-id" style="color:#6b7280;font-size:9px">(ID: ?)</span></div>
                <div id="e2h-nearby-maps" style="max-height:120px;overflow-y:auto"></div>
            </div>

            <!-- Start/Stop + Idź na E2 -->
            <div class="e2h-btns">
                ...
                <button class="e2h-btn" id="e2h-goto-e2" style="background:linear-gradient(135deg,#7c3aed,#2563eb);flex:1" title="Nawiguj do mapy E2">🚀 Idź na E2</button>
            </div>
        ...
        // Idź na E2
        document.getElementById('e2h-goto-e2').addEventListener('click', async () => {
            const mapInfo = getMapInfo();
            // Sprawdź czy już jesteśmy na mapie E2
            if (E2_RETURN_ROUTES[mapInfo.id]) {
                log(`✅ Już jesteś na mapie E2: ${mapInfo.name}!`, 'ok');
                return;
            }
            log(`🚀 Ręczna nawigacja na E2 uruchomiona!`, 'warn');
            await activateReturnToE2();
            // Upewnij się że bot jest włączony (potrzebny do nawigacji)
            if (!botRunning) {
                log(`⚡ Uruchamiam bota, żeby nawigować...`, 'info');
                startBot();
            }
        });
        ...
        const toggles = [
            ...
            ['e2h-autoReturnToE2', 'autoReturnToE2'],
        ...
        // Collapsible character boss table toggle
        const toggleHeader = document.getElementById('e2h-toggle-char-boss-table');
        const container = document.getElementById('e2h-char-boss-container');
        const arrow = document.getElementById('e2h-char-boss-arrow');
        const rowsContainer = document.getElementById('e2h-char-boss-rows');

        window.renderCharBossTable = function() {
            if (!rowsContainer) return;

            if (!lastAccountCharacters || lastAccountCharacters.length === 0) {
                rowsContainer.innerHTML = '<div style="font-size:10px;color:#64748b;padding:4px 0">Brak postaci. Spróbuj za chwilę...</div>';
                return;
            }

            rowsContainer.innerHTML = '';
            
            // Pobierz unikalne posortowane nazwy bossów z E2_DATABASE
            const bossList = [...new Set(E2_DATABASE.map(e => e.name))].sort();
            
            let mapping = {};
            if (_serverConfig && _serverConfig.char_boss_map) {
                try {
                    mapping = typeof _serverConfig.char_boss_map === 'string'
                        ? JSON.parse(_serverConfig.char_boss_map)
                        : _serverConfig.char_boss_map;
                } catch {
                    mapping = {};
                }
            }

            lastAccountCharacters.forEach(char => {
                const nick = char.nick;
                const currentBoss = mapping[nick.toLowerCase()] || '';
                ...
                info.textContent = nick;

                const select = document.createElement('select');
                select.style.background = '#111';
                select.style.color = '#fff';
                select.style.border = '1px solid rgba(59,130,246,0.3)';
                ...
                if (!currentBoss) optNone.selected = true;
                ...
                bossList.forEach(boss => {
                    ...
                    if (currentBoss.toLowerCase() === boss.toLowerCase()) opt.selected = true;
                ...
                select.addEventListener('change', async function() {
                    ...
                    if (_serverConfig && _serverConfig.char_boss_map) {
                        try {
                            latestMapping = typeof _serverConfig.char_boss_map === 'string'
                                ? JSON.parse(_serverConfig.char_boss_map)
                                : _serverConfig.char_boss_map;
                        } catch {
                            latestMapping = {};
                    ...
                    if (!selectedBoss) {
                        delete latestMapping[nick.toLowerCase()];
                    } else {
                        latestMapping[nick.toLowerCase()] = selectedBoss;
                    }
                    ...
                    if (window.MAW_NODE_API) {
                        try {
                            const res = await fetch(`${window.MAW_NODE_API}/api/config`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ char_boss_map: latestMapping })
                            });
                            if (res.ok) {
                                _serverConfig.char_boss_map = latestMapping;
                                log(`🎯 Przypisano ${selectedBoss || 'Brak'} do postaci ${nick}`, 'ok');
                            }
                        } catch (e) {
                            console.error('[E2H] Error saving mapping:', e);
                        }
                    }
                });
    ...
    function renderPhase() {
        ...
        const phases = {
            ...
            returning: ['🔄', `Powrót na E2...`, 'on']
        };
        if (_needToReturnToE2 && _returnPhase === 'returning') phase = 'returning';
    ...
    let _lastNearbyRender = 0;
    function renderNearbyMaps() {
        const now = Date.now();
        if (now - _lastNearbyRender < 2000) return; // nie renderuj częściej niż co 2s
        _lastNearbyRender = now;

        const mapEl = document.getElementById('e2h-current-map');
        const mapIdEl = document.getElementById('e2h-current-map-id');
        const listEl = document.getElementById('e2h-nearby-maps');
        if (!listEl) return;

        const mapInfo = getMapInfo();
        if (mapEl) mapEl.textContent = mapInfo.name;
        if (mapIdEl) mapIdEl.textContent = `(ID: ${mapInfo.id})`;

        const gateways = scanGateways();

        if (gateways.length === 0) {
            listEl.innerHTML = '<div style="padding:6px;text-align:center;color:#475569;font-size:10px">Brak przejść na tej mapie</div>';
            return;
        }

        listEl.innerHTML = gateways.map((g, i) => {
            // Sprawdź czy ta mapa jest na trasie E2
            const isOnRoute = Object.values(E2_RETURN_ROUTES).some(r => r.route.some(rm => rm.id === g.targetMapId));
            const routeTag = isOnRoute ? '<span style="color:#7c3aed;font-weight:bold;margin-left:3px">⚡E2</span>' : '';
            return `<div class="e2h-gate-row" data-gate-idx="${i}" style="
                display:flex;align-items:center;justify-content:space-between;
                padding:3px 6px;margin:2px 0;border-radius:4px;
                background:${isOnRoute ? 'rgba(124,58,237,0.15)' : 'rgba(255,255,255,0.03)'};
                cursor:pointer;transition:background 0.2s;
            ">
                <div style="flex:1;min-width:0">
                    <span style="color:#e2e8f0;font-size:10px;font-weight:500">${g.name}</span>${routeTag}
                    <div style="color:#6b7280;font-size:8px">ID: ${g.targetMapId} • [${g.tx}, ${g.ty}]</div>
                </div>
                <button class="e2h-gate-btn" data-gate-idx="${i}" style="
                    background:linear-gradient(135deg,#2563eb,#1d4ed8);color:white;
                    border:none;border-radius:3px;padding:2px 8px;font-size:9px;
        ...
        listEl.querySelectorAll('.e2h-gate-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                const idx = parseInt(btn.getAttribute('data-gate-idx'));
                const gate = gateways[idx];
                if (gate) {
                    log(`🚶 Ręczna nawigacja A* → ${gate.name} [${gate.tx}, ${gate.ty}]`, 'warn');
                    startWalkToGateway(gate.tx, gate.ty);
                    if (!botRunning) {
                        log(`⚡ Uruchamiam bota, aby wykonać drogę...`, 'info');
                        startBot();
                    }
                }
            });
        ...
        // Hover effect
        listEl.querySelectorAll('.e2h-gate-row').forEach(row => {
            row.addEventListener('mouseenter', () => row.style.background = 'rgba(37,99,235,0.2)');
            row.addEventListener('mouseleave', () => {
                const idx = parseInt(row.getAttribute('data-gate-idx'));
                const gate = gateways[idx];
                const isOnRoute = gate && Object.values(E2_RETURN_ROUTES).some(r => r.route.some(rm => rm.id === gate.targetMapId));
                row.style.background = isOnRoute ? 'rgba(124,58,237,0.15)' : 'rgba(255,255,255,0.03)';
            });
        });
    }
...
async function clickReloggerCharacter(targetCharName) {
    ...
    if (!relogger) return false;

    // Tymczasowo uaktywnij wszystkie grupy światów
    const charGroups = relogger.querySelectorAll('.relogger__char-group');
    const prevGroupStates = [];
    charGroups.forEach(g => {
        prevGroupStates.push({ el: g, display: g.style.display, active: g.classList.contains('active') });
    ...
    const chars = relogger.querySelectorAll('.relogger__one-character');
    let targetEl = null;
    ...
    for (const char of chars) {
        ...
        if (!tipText) {
            ...
            await delay(60);
            const tooltip = document.querySelector('.tip-wrapper, .normal-tip, #tooltip, .tooltip, .tip-content');
            if (tooltip) tipText = tooltip.textContent.trim();
            char.dispatchEvent(new MouseEvent('mouseleave', { bubbles: true }));
        }

        if (tipText.toLowerCase().includes(cleanTarget)) {
            targetEl = char;
            break;
        }
    }
    ...
    if (targetEl) {
        // Kliknij grupę świata jeśli nie była aktywna
        const parentGroup = targetEl.closest('.relogger__char-group');
        const world = parentGroup ? parentGroup.getAttribute('data-world') : null;
        if (world) {
            const worldBtn = relogger.querySelector(`.relogger__one-world[data-world="${world}"]`);
        ...
        // Przywróć stany grup jeśli nie znaleziono
        prevGroupStates.forEach(s => {
            s.el.style.display = s.display;
            if (!s.active) s.el.classList.remove('active');
        });
...
// ═══════════════════════════════════════════════════════════════
//  AUTO RETURN TO E2 — sterowane z dashboardu
//  Używa tej samej logiki co przycisk "Idź na E2" w UI
// ═══════════════════════════════════════════════════════════════
async function tryReturnToE2() {
    if (!window.AUTO_RETURN_TO_E2) return;
    if (_returnPhase === 'returning') return;

    const heroNick = getHeroNick ? getHeroNick() : '';
    if (!heroNick) return;

    console.log(`[E2H] Auto Return (z dashboardu) → bohater: ${heroNick}`);

    // Użyj tej samej funkcji co przycisk w UI (activateReturnToE2 lub podobna)
    if (typeof activateReturnToE2 === 'function') {
        console.log('[E2H] Wywołuję activateReturnToE2() — tę samą co z UI');
        activateReturnToE2();   // <--- to jest klucz!
        _returnPhase = 'returning';
    } else {
        // Fallback jeśli nie ma tej funkcji
        log('⚠️ Nie znaleziono activateReturnToE2 — używam podstawowej trasy', 'warn');
        const currentMapId = window.Engine?.map?.d?.id;
        const routeInfo = Object.values(E2_RETURN_ROUTES).find(r => r.route.some(s => s.id == currentMapId));
        if (routeInfo) {
            for (const step of routeInfo.route) {
                if (window.Engine.map.d.id == step.id) continue;
                log(`🚶 Przechodzę: ${step.name}`, 'info');
                await delay(1600);
            }
        }
    }
...
// ==================== PUSH TIMERÓW DO RUNBOT (Node) ====================
function pushTimersToServer() {
    if (!window.MAW_NODE_API) return;

    const timers = [];
    document.querySelectorAll('.elite-timer .row, .elite-timer-wnd .row, .npc-list .row').forEach(row => {
        const nameEl = row.querySelector('.name-val, .name');
        const timeEl = row.querySelector('.time-val, .time');
        ...
        else seconds = parseInt(timeStr) || 9999;
    ...
    if (timers.length > 0) {
        ...
        }).catch(err => console.warn('[E2H] Błąd push timerów:', err));
    }
}

// Uruchom co 8 sekund
setInterval(pushTimersToServer, 8000);

// Pierwsze uruchomienie
setTimeout(pushTimersToServer, 5000);

console.log('[E2H] Timer push do Node włączony');
    ...
    waitForEngine(async () => {
        ...
        // Natychmiast pobierz konfigurację z serwera (char_boss_map, ustawienia E2, itp.)
        await fetchConfig();
        buildStyles();
        buildPanel();
        startRenderLoop();
        ...
        // ═══ ANTI-STASIS: OSOBNY INTERVAL (działa ZAWSZE, nawet bez bota!) ═══
        setInterval(() => {
            try {
                // Szukaj OBU overlayów: ostrzeżenie + sam stan nieaktywności
                ...
                // STASIS WYKRYTY — robimy krok
                console.log('[E2H:Anti-Stasis] ⚡ Wykryto stasis/ostrzeżenie — ruszam postacią!');
                const h = window.Engine?.hero?.d;
                if (h && window.Engine?.hero?.autoGoTo) {
                    const dx = Math.floor(Math.random() * 3) - 1;
                    const dy = Math.floor(Math.random() * 3) - 1;
                    window.Engine.hero.autoGoTo({ x: h.x + (dx || 1), y: h.y + (dy || 1) });
                    log(`⚡ ANTI-STASIS: Ruch [${h.x + (dx||1)}, ${h.y + (dy||1)}]`, 'warn');
                }
            } catch (e) {
                console.error('[E2H:Anti-Stasis] Błąd:', e);
            }
        }, 2000);
